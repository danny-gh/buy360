...
async function myReward(){
  return new Promise(async (resolve) => {
    ...
    let options = {
      "url": `https://sendbeans.jd.com/common/api/bean/activity/myReward?itemsPerPage=10&currentPage=1&sendType=0&invokeKey=${invoke_key}`,
      "headers": {
...
async function getActivityList(){
  return new Promise((resolve) => {
    ...
    let lks = $.md5('' + invoke_key + lkt).toString()
    let options = {
      "url": `https://sendbeans.jd.com/common/api/bean/activity/get/entry/list/by/channel?channelId=14&channelType=H5&sendType=0&singleActivity=false&invokeKey=${invoke_key}`,
      "headers": {
        "Host": "sendbeans.jd.com",
        "Origin": "https://sendbeans.jd.com",
        "Cookie": $.cookie,
...
async function rewardBean(){
  return new Promise((resolve) => {
    ...
    let lks = $.md5('' + invoke_key + lkt).toString()
    let options = {
      "url": `https://draw.jdfcloud.com/common/api/bean/activity/sendBean?rewardRecordId=${$.rewardRecordId}&jdChannelId=&userSource=mp&appId=wxccb5c536b0ecd1bf&invokeKey=${invoke_key}`,
      "headers":  {
...
async function help() {
  await new Promise((resolve) => {
    let lkt = new Date().getTime()
    let lks = $.md5('' + invoke_key + lkt + $.activityCode).toString()
    let options = {
      "url": `https://draw.jdfcloud.com/common/api/bean/activity/participate?activityCode=${$.activityCode}&activityId=${$.activityId}&inviteUserPin=${encodeURIComponent($.oneTuanInfo['user'])}&invokeKey=${invoke_key}&timestap=${Date.now()}`,
      "headers":  {
        'content-type' : `application/json`,
        'Connection' : `keep-alive`,
        'Accept-Encoding' : `gzip,compress,br,deflate`,
...
async function invite() {
  let lkt = new Date().getTime()
  let lks = $.md5('' + invoke_key + lkt + $.activityCode).toString()
  const url = `https://draw.jdfcloud.com/common/api/bean/activity/invite?activityCode=${$.activityCode}&openId=&activityId=${$.activityId}&userSource=mp&formId=123&jdChannelId=&fp=&appId=wxccb5c536b0ecd1bf&invokeKey=${invoke_key}`;
  const method = `POST`;
  const headers = {
    'content-type' : `application/json`,
    'Connection' : `keep-alive`,
...
async function getActivityDetail() {
  let lkt = new Date().getTime()
  let lks = $.md5('' + invoke_key + lkt + $.activityCode).toString()
  const url = `https://draw.jdfcloud.com/common/api/bean/activity/detail?activityCode=${$.activityCode}&activityId=${$.activityId}&userOpenId=&timestap=${Date.now()}&userSource=mp&jdChannelId=&appId=wxccb5c536b0ecd1bf&invokeKey=${invoke_key}`;
  const method = `GET`;
  const headers = {
    'cookie' : $.cookie,
    'openId' : ``,
