          ...
          if (safeGet(data)) {
            ...
            if (data.code === '0') {
              if (data.result.taskDone === 0) {
                await $.wait(1500)
                await goldCreatorDoTask({ "type": 1, "log": "", "random":`${getRandomInt(20000000,30000000)}`, "sceneid": "jdgoldenranking2022h5"})
              }
            } else {
...
function goldCreatorDoTask(body) {
  return new Promise(resolve => {
    const options = postUrl('goldCenterDoTask', body)
    // console.log(options);
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(JSON.stringify(body))}&appid=content_ecology&clientVersion=10.2.4&client=wh5&ext={"prstate":"0"}&uuid=${$.UUID}`,
    headers: {
      "Accept": "*/*",
...
function postUrl(function_id, body = {}) {
    return {
        url: `${JD_API_HOST}`,
        body: `functionId=${function_id}&body=${escape(JSON.stringify(body))}&appid=content_ecology&clientVersion=10.2.4&client=wh5&ext={"prstate":"0"}&uuid=${$.UUID}`,
        headers: {
            'Host': 'api.m.jd.com',
            'content-type': 'application/x-www-form-urlencoded',
